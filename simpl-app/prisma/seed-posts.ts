...
export async function createPosts(prisma: PrismaClient, actors: SeedActors) {
  ...
  const replyPosts = await Promise.all([
    ...
    createPost(prisma, { title: "Signalement en discussion", body: "Le contenu est limite, mais pas forcément à masquer. Ça mérite une vraie revue collective.", authorId: alice.id, parentId: rootPosts[9].id, rootId: rootPosts[9].id, latitude: 46.5218, longitude: 6.6298 }),
